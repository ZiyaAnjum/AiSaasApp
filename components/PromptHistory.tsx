...
export default function PromptHistory() {
  ...
  }, [token, selectedToolFilter, searchQuery, favoritesOnly, showToast]);
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-200 pb-6 dark:border-zinc-800">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
            Search, review, bookmark, and export your previous AI prompts and generated results.
          </p>
        </div>
      ...
      <div className="mb-6 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
        <form onSubmit={handleSearchSubmit} className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search within prompts, responses, or tools..."
            value={searchQuery}
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="mt-3 text-xs text-zinc-500">Loading prompt history...</p>
        </div>
      ) : history.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-zinc-300 p-12 text-center dark:border-zinc-800">
          ...
          <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400 max-w-sm mx-auto">
            {searchQuery || favoritesOnly || selectedToolFilter !== 'all'
              ? 'No records match your active search filters. Try adjusting your search query.'
              : 'Start executing requests in the AI Tools Studio to see your prompt records logged here.'}
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          {history.map((item) => (
            <div
              key={item.id}
              className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm transition-all hover:border-zinc-300 dark:border-zinc-800 dark:bg-zinc-900 dark:hover:border-zinc-700"
            >
              {/* Item Header */}
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-zinc-100 pb-3 dark:border-zinc-800">
                <div className="flex items-center gap-2">
                  ...
                  {item.source === 'api' && (
                    <span className="rounded bg-rose-50 px-1.5 py-0.5 text-[10px] font-semibold text-rose-700 dark:bg-rose-950 dark:text-rose-300">
                      API Gateway
                    </span>
                  )}
                </div>
